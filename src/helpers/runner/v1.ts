 ...
 * step so far (`steps.<id>....`) and the memory as it stands. The rules
 * themselves -- a lone expression keeping its type, an empty value never
 * shadowing what an earlier step remembered -- live in `helpers/memory`,
 * because a latent application resolves its own mapping by exactly the same
 * ones.
 *
  ...
  try {
    ...
    for (let i = 0; i < flow.steps.length; i++) {
      ...
      try {
        ...
        flow.steps[i].execution.times.duration =
          (flow.steps[i].execution.times.end - flow.steps[i].execution.times.start) / 1000;
        flow.reporter.stepUpdate(id);
